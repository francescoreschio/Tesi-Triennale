...
Il criterio che utilizziamo per affermare che vi è un \textit{match} tra un candidato muone emulato e un muone del GMT del sistema di Data Scouting è studiare la distanza tra due muoni nello stesso BX nel piano $\phi - \eta$, ovvero:
\begin{equation}
  \label{eq:DeltaR}
  \Delta R = \sqrt{(\Delta \phi)^2 + (\Delta \eta)^2}
\end{equation}

Dove $\Delta \phi = \phi_{BMTF} - \phi_{GMT}$ e $\Delta \eta = \eta_{BMTF} - \eta_{GMT}$. Un buon 
